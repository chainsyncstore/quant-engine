...
The promotion updates `registry/active.json`, marks the promoted record `status=active`, records `promoted_at`, `promoted_by`, and promotion notes, then reloads the runtime signal source. If runtime loading fails, the bot attempts to restore the prior active pointer.
