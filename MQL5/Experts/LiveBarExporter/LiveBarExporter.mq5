...
input string         InpOutputFile   = "results\\live_eurusd.csv";
...
int OnInit()
{
   ...
   if(!LoadLastExportedTimestamp())
      return INIT_FAILED;

   int timer_period = MathMax(InpTimerSeconds, 1);
   EventSetTimer(timer_period);

   string last_label = g_last_exported_bar > 0
      ? TimeToString(g_last_exported_bar, TIME_DATE | TIME_MINUTES)
      : "none";
   PrintFormat("LiveBarExporter initialized | file=%s tf=%d last_bar=%s", InpOutputFile, InpTF, last_label);
...
void OnTimer()
{
   const int bars = iBars(_Symbol, InpTF);
   if(bars < 2)
      return;

   datetime bar_time = iTime(_Symbol, InpTF, 1); // last CLOSED bar
   if(bar_time <= g_last_exported_bar)
      return;

   double open  = iOpen(_Symbol, InpTF, 1);
   double high  = iHigh(_Symbol, InpTF, 1);
   double low   = iLow(_Symbol, InpTF, 1);
   double close = iClose(_Symbol, InpTF, 1);
   long volume  = (long)iVolume(_Symbol, InpTF, 1);
   ...
   if(AppendLineAtomically(InpOutputFile, line))
   {
      ...
      PrintFormat("live_bar_exported | symbol=%s time=%s", _Symbol, timestamp);
   }
}
...
bool LoadLastExportedTimestamp()
{
   ...
   string fields[];
   const int count = StringSplit(last_line, ',', fields);
   if(count < 2)
      return true;

   datetime parsed = StringToTime(fields[1]);
   if(parsed > 0)
      g_last_exported_bar = parsed;
